...
func FSizeW(_ size:CGFloat) ->CGFloat {
    return size * (ScreenWidth / 375)
}

func FSizeH(_ size:CGFloat) ->CGFloat{
...
func GetReadTableViewFrame() ->CGRect {
    ...
    if isX {
        // Y = 刘海高度 + 状态View高 + 间距
        let y =  TopLiuHaight + FSpace_2 + FSpace_6
        let bottomHeight = TopLiuHaight
        return CGRect(x: FSpace_1, y: y, width: ScreenWidth - 2 * FSpace_1, height: ScreenHeight - y - bottomHeight)
    }else{
        // Y =  状态View高 + 间距
        let y =  FSpace_2 + FSpace_6
        return CGRect(x: FSpace_1, y: y, width: ScreenWidth - 2 * FSpace_1, height: ScreenHeight - 2 * y)
    }
}
...
/// 主文件夹名称
private let ReadFolderName:String = "FBookRead"

/// 归档阅读文件文件
func BookKeyedArchiver(folderName:String,fileName:String,object:AnyObject) {
...
/// 解档阅读文件文件
func BookKeyedUnarchiver(folderName:String,fileName:String) ->AnyObject? {
...
/// 是否存在了该归档文件
func BookKeyedIsExistArchiver(folderName:String,fileName:String? = nil) ->Bool {
