...
@objc protocol FSegmentedControlDelegate:NSObjectProtocol {
    
    /// 点击按钮
    @objc optional func segmentedControl(segmentedControl:FSegmentedControl, clickButton button:UIButton, index:NSInteger)
}

class FSegmentedControl: UIView {

    /// 代理
    weak var delegate:FSegmentedControlDelegate?
    ...
    init(_ delegate:FSegmentedControlDelegate? = nil) {
    ...
    private func addSubviews() {
        ...
        for i in 0..<count {
            ...
            button.addTarget(self, action: #selector(FSegmentedControl.clickButton(button:)), for: .touchDown)
