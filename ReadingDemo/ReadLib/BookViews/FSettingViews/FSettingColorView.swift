...
class FSettingColorView: FBaseView {
    ...
    /// 初始化方法
    init(frame:CGRect,readMenu:FReadMenu,colors:[UIColor],selectIndex:NSInteger) {
    ...
    /// 选中按钮
    private var selectButton:FHaloButton?
    ...
    override func addSubviews() {
        ...
        let PublicButtonWH:CGFloat = FHaloButton.HaloButtonSize(CGSize(width: 39, height: 39)).width
        
        let spaceW:CGFloat = (ScreenWidth - 2 * FSpace_1 - CGFloat(count) * PublicButtonWH) / CGFloat(count - 1)
        
        for i in 0..<count {
            
            let color = colors[i]
            
            let publicButton = FHaloButton(CGRect(x: FSpace_1 + CGFloat(i) * (PublicButtonWH + spaceW), y: FSpace_1, width: PublicButtonWH, height: PublicButtonWH), haloColor:color)
    ...
    /// 点击按钮
    @objc func clickButton(button:FHaloButton) {
